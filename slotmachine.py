...
def get_bet():
    while True:
        amount = input("Enter Bet amount on each line, Ksh: ")
        if amount.isdigit():
            amount = int(amount)
...
def spin(balance):
    ...
    while True:
        ...
        if total_bet > balance:
            print(
                f"You do not have sufficient funds to make this bet, your balance is Ksh.{balance}")
        else:
            break    

    print(
        f"You are betting Ksh.{bet} on {lines} lines. Total bet is equal to: Ksh.{total_bet}")
